...
public class VipsSeparatorNonGraphicsDetector implements VipsSeparatorDetector {
	...
	private int _cleanSeparatorsThreshold = 0;
	...
	public void detectHorizontalSeparators()
	{
		...
		if (_cleanSeparatorsThreshold != 0)
	...
	private void cleanUpSeparators(List<Separator> separators)
	{
		...
		for (Separator separator : tempList)
		{
			...
			if (width < _cleanSeparatorsThreshold)
	...
	/**
	 * If a visual separator is overlapped with some certain HTML tags, its weight is set to be higher.
	 */
	...
	/**
	 * For horizontal separators, if the differences of font properties such as font size and font weight are bigger on two
	 * sides of the separator, the weight will be increased.
	 * Moreover, the weight will be increased if the font size of the block above the separator is
	 * smaller than the font size of the block below the separator.
	 */
	...
	/**
	 * For horizontal separators, when the structures of the blocks on the two sides of the separator are very similar,
	...
	{
		this._cleanSeparatorsThreshold = treshold;
	}
